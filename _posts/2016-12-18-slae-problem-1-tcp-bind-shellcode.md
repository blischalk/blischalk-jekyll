...
This blog post has been created for completing the requirements foe
...
[TCP Bindshell Source and Tools](https://github.com/blischalk/slae/tree/master/exercise1)
...
{% highlight bash %}
root@blahblah:~# netstat -atp Active Internet connections (servers and established)
...
We can use a tool called @strace@ to help us learn more about what
...
and filtering out the noise we will see:
...
Ok. It looks like our code makes some system calls that seem to align
...
~execve~ call to run /bin/sh.

Lets lookup the system calls to find out their system call numbers. We
will consult ~/usr/include/i386-linux-gnu/asm/unistd_32.h~ for these
numbers.

When we consult the listing of syscalls we encounter a bit of
confusion. The only system calls that seem to closely match up with
what we saw in our strace are:
...
This is strange. ~execve~ and ~dup2~ look good but there doesn't seem
to be any syscall numbers for socket,listen, or accept. socketcall
seems a bit odd as does mbind so we will have to look into this.
...
function signatures look like the following:
...
We also leveraged 2 structs in our C program which we will most likely
need to replicate.
...
Ok. Using what we have gathered from our analysis lets take an attempt
at writing some assembly!
...
I had a hard time finding where ~SOCK_STREAM~ was defined so we use a
little gcc magic to see what the macro expands to:
...
Bingo! Our assembly works and gives us a tcp bind shell. Now we need
to test it in our c program stub. We will use some command line fu to
get the opcodes from our binary:
...
Once we have our script we print out our updated shellcode and pop it
